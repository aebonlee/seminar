...
export function Login() {
  ...
  return (
    <SubPage title="로그인" description="이메일 또는 소셜 계정으로 로그인하세요." breadcrumb={[{ label: 'Account' }, { label: '로그인' }]}>
      <div style={{ maxWidth: 460, margin: '0 auto' }}>
        <SubPanel>
          <button type="button" className="btn btn-oauth btn-google" onClick={() => oauth('google')} style={{ width: '100%', marginBottom: 10 }} disabled={!backendReady}>
            <GoogleIcon /> Google로 계속하기
          </button>
          <button type="button" className="btn btn-oauth btn-kakao" onClick={() => oauth('kakao')} style={{ width: '100%' }} disabled={!backendReady}>
            <KakaoIcon /> 카카오로 계속하기
          </button>
          {!backendReady && (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '24px 0' }}>
            ...
            <span style={{ color: '#94a3b8', fontSize: 12, fontWeight: 700 }}>또는</span>
            <span style={{ flex: 1, height: 1, background: '#e2e8f0' }} />
          </div>
          ...
          <form onSubmit={submit}>
            <div className="form-group">
              ...
              <input id="email" type="email" className="form-input" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="you@example.com" />
            </div>
            <div className="form-group">
          ...
          <p style={{ marginTop: 20, textAlign: 'center', color: '#64748b', fontSize: 14 }}>
            계정이 없으신가요? <Link to="/signup" style={{ color: 'var(--accent-700)', fontWeight: 700 }}>회원가입</Link>
          </p>

          {!backendReady && (
            <div style={{ marginTop: 24, padding: 16, background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--accent-700)', letterSpacing: '0.14em' }}>
                데모 모드
              </div>
              <p style={{ marginTop: 6, marginBottom: 12, color: '#64748b', fontSize: 13 }}>
